# res://scripts/weapons/RocketWeapon.gd
extends ShooterWeapon
class_name RocketWeapon

@onready var pd = get_tree().root.get_node("PlayerData")

@export var missile_scene: PackedScene = preload("res://scenes/bullets/Missile.tscn")
@export var base_expl_damage: float = 60.0
@export var base_radius:      float = 64.0
# set inherited base_fire_rate to 0.7 in the Inspector

func _damage_type_key() -> String:
	return "explosive_damage_percent"

# ─── use the passed-in player_data instead of onready pd here
func apply_weapon_modifiers(player_data: PlayerData) -> void:
	super.apply_weapon_modifiers(player_data)
	var atk_speed: float = player_data.get_stat("attack_speed")
	final_fire_rate = base_fire_rate * (0.5 + 0.5 * atk_speed)
